...
class MainMenu:
    ...
    def run(self):
        ...
        cursorManager = CursorManager(
            'Games/assets/cursorRight.png',
            'Games/assets/cursorLeft.png')
        tracker = HandTrackerWrapper()
        uiManager = UiManager()
        bg_image = cv2.resize(
            cv2.imread('Games/assets/mainMenuBG.png'),
        ...
        SimpleDrawGameButton.CreateImageButton(
            cv2.imread('Games/assets/freestyle_button.jpg',
                       cv2.IMREAD_UNCHANGED))
        bg_image_main = None
        cv2.namedWindow("MainMenu", cv2.WINDOW_GUI_NORMAL)
        cv2.resizeWindow('MainMenu', bg_image.shape[1], bg_image.shape[0])
